...
class EmployeRepository extends ServiceEntityRepository
{
    ...
    /**
    * @return Employe[] Returns les employes
    */
    
    public function searchEmploye($employeToSearch)
    {
        $employe = "%" . trim($employeToSearch) . "%";
        return $this->createQueryBuilder('e')
            ->where('e.nom LIKE :texte')
            //->where('s.typeDeService LIKE \'%Net%\'')
            ->orderBy('e.nom', 'ASC')
            // ->setMaxResults(10)
            ->setParameter('texte', $employe)
            ->getQuery()
            ->getResult()
        ;
    }
